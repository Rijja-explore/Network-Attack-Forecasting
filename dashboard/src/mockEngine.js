...
// Backward-compatible alias for both naming conventions
MOCK_SCENARIOS.zero_day = MOCK_SCENARIOS.zeroday;
...
export function generateOfflineReportForFile(fileName) {
  ...
  let base = MOCK_SCENARIOS.neris_c2;
  if (lower.includes('benign') || lower.includes('normal')) base = MOCK_SCENARIOS.benign;
  else if (lower.includes('recon') || lower.includes('scan') || lower.includes('port')) base = MOCK_SCENARIOS.recon;
  else if (lower.includes('brute') || lower.includes('ssh') || lower.includes('auth')) base = MOCK_SCENARIOS.bruteforce;
  else if (lower.includes('ddos') || lower.includes('flood') || lower.includes('syn')) base = MOCK_SCENARIOS.ddos;
  else if (lower.includes('zero') || lower.includes('novel') || lower.includes('unknown')) base = MOCK_SCENARIOS.zeroday;

  // Clone and inject realistic uploaded file metadata
  return {
    ...base,
    filename: fileName,
